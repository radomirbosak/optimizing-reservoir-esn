...
Goal: Calculate the memory capacity depending on largest |eigenvalue|
...
from numpy import sum, random, meshgrid, zeros, \
	linspace, average, std, sqrt, linalg, max, argmax, abs
...
svals = sorted(list(linspace(0.75, 1.1, 20)) + list(linspace(0.95, 1.01, 10)))
np.savetxt('saved/eigs.txt', svals)
...
for qi, q in enumerate(res_sizes):
	...
	for i, sval in enumerate(svals):
		...
		for inm in range(INSTANCES):
			...
			current_sv = max(abs(linalg.eig(W)[0]))
			W = W * (sval / current_sv)
