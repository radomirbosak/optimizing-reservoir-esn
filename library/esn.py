...
class ESN():
	def __init__(self, input, hidden, output):
		...
		self.gen_w(0.95)
		self.gen_wi(0.5)
		self.WO = np.zeros([output, input + hidden])
	...
	def train(self, training_set, length, verbose=False):
		...
		S = np.zeros([p + q, iterations_measured]) # dlhy obdlznik
		D = np.zeros([r, iterations_measured])

		for it, (input, desired) in enumerate(training_set):
			self.input = input
			self.reservoir()
			S[:, it] = np.hstack((self.input, self.X))
			D[:, it] = desired
	...
	def reoutput(self):
		self.output = np.dot(self.WO, np.hstack((self.input, self.X)))